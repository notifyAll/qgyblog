...
public class TimeUtil {
	 ...
	 * @return
	 */
	public static Date getSunday(Date date, int num) {
		date = new Date(date.getTime() + 1L * num * WEEK);
		int weekNum = date.getDay();
		return new Date(date.getTime() + 1L * weekNum * DAY);
	}
	 ...
	 * @return
	 */
	public static int differ2date(String toDate, Date formDAte) {
		SimpleDateFormat df = new SimpleDateFormat(TIME_DATE);
		SimpleDateFormat df2 = new SimpleDateFormat(TIME_SAMPLE);

		long to = 0;
		long from = 0;
		try {
			to = df.parse(df.format(df2.parse(toDate))).getTime();
			from = df.parse(df.format(formDAte)).getTime();
	 ...
	 * @return
	 */
	public static int differ2date(Date toDate, String formDAte) {
		SimpleDateFormat df = new SimpleDateFormat(TIME_SAMPLE);

		long to = 0;
		long from = 0;
		try {
			to = df.parse(df.format(toDate)).getTime();
			from = df.parse(formDAte).getTime();
		} catch (ParseException e) {
			e.printStackTrace();
		}
	...
	// try {
	// return
	// StringToDate(addDayWork(dateToString(date,TIME_SAMPLE),1),TIME_SAMPLE);
	// } catch (Exception e) {
	// e.printStackTrace();
	// return null;
	// }
	...
	// // String holidays =
	// "2015-01-01,2015-01-02,2015-01-03,2015-04-04,2015-04-05,2015-04-06,2015-05-01,2015-05-02,2015-05-03,2015-06-20,2015-06-21,2015-06-22,2015-09-26,2015-09-27,2015-10-01,2015-10-02,2015-10-03,2015-10-04,2015-10-05,2015-10-06,2015-10-07";
	// //为空，则没设置节假日
	...
	// boolean isHoliday =
	// org.apache.commons.lang.ArrayUtils.contains(holidayArray, dateString);
	// return isHoliday;
	// }
	...
	public static String getTimestamp() throws ParseException{  
        ...
        Date date = dateFormat.parse(getNow());
        long times = date.getTime();
        return Long.toString(times);
    }  
	public static long getTimestampInt() throws ParseException{  
        DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date date = dateFormat.parse(getNow());
        return date.getTime();
    }
	
    public static String changeTimestamp(long times){  
//    	long times = 1488769080000L;
        Date date = new Date(times);
        SimpleDateFormat bartDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        return bartDateFormat.format(times);
	...
	public static void main(String[] strings) {
		...
		Long epTime = 1477840400000l;

		Date date = new Date(epTime);

		System.out.println(changeTimestamp(epTime));
//		System.out.println(dateToString(date, "yyyy-MM-dd HH:mm:ss"));

//		String time = "2016-04-30 10:41:57";
//
//		System.out.println(StringToDate(time, "yyyy-MM-dd HH:mm:ss").getTime());

//		String st = TimeUtil.getNow();
